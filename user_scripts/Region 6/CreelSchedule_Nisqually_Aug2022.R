...
# Specify start and end dates for creel survey schedule (format "yyyy-mm-dd")
  ui_startDate<-as.Date(c("2022-08-01")) 
  ui_endDate<-as.Date(c("2022-08-31")) 
...
# If needed, specify days of the week and/or individual dates when the (entire) fishery will be closed (NOTE: do NOT enter any dates where only portions of the fishery will closed.  Here adjustments will need to be made to data collection protocol)
  ui_closed_weekdays<-c("Sunday", "Monday", "Tuesday") # If applicable, list weekdays that the fishery will be closed; Example: c("Sunday", "Monday")
  ui_closed_start_end<-c("2022-08-07", "2022-08-31")   # If certain weekdays are closed, enter the start and end dates for closures; Example: c("2022-08-07", "2022-08-31")
  ui_closed_dates<-c("2022-08-24", "2022-08-31")       # If applicable, list individual closure dates; NOTE: these can be in addition to any "ui_closed_weekdays"; Example: c("2022-08-24", "2022-08-31")   
...
# Sample location(s) 
    ...
    ui_water_bodies<-c("Nisqually River") # Enter the waterbody(s) that will be surveyed (case sensitive and must match water_body listed in lut$water_body
