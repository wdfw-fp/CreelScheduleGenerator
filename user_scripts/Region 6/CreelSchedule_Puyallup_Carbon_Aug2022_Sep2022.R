...
# Specify start and end dates for creel survey schedule (format "yyyy-mm-dd")
  ui_startDate<-as.Date(c("2022-08-16")) 
  ui_endDate<-as.Date(c("2022-09-30")) 
...
# If needed, specify days of the week and/or individual dates when the (entire) fishery will be closed (NOTE: do NOT enter any dates where only portions of the fishery will closed.  Here adjustments will need to be made to data collection protocol)
  ...
  ui_closed_start_end<-c("2022-08-16", "2022-09-30")   # If certain weekdays are closed, enter the start and end dates for closures; Example: c("2022-08-07", "2022-08-31")
...
#---------------------------------------------------------------------------------------------------------------------------- - 
# Sample location(s) 
    lut$water_body |> print(n=Inf) # List of water bodies and their associated lat/long data that have been entered into the "master" LUT list
    ui_water_bodies<-c("Puyallup River", "Carbon River") # Enter the waterbody(s) that will be surveyed (case sensitive and must match water_body listed in lut$water_body
