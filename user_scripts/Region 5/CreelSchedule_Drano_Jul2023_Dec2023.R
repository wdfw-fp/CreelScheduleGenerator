...
# If needed, specify days of the week and/or individual dates when the (entire) fishery will be closed (NOTE: do NOT enter any dates where only portions of the fishery will closed.  Here adjustments will need to be made to data collection protocol)
  ...
  ui_closed_start_end<-c("2022-10-05", "2022-10-26")   # If certain weekdays are closed, enter the start and end dates for closures; Example: c("2022-08-07", "2022-08-31")
...
# Sample location(s) 
    ...
    ui_water_bodies<-c("Drano Lake") # Enter the water bodies that will be surveyed (case sensitive and must match water_body listed in lut$water_body
...
# Estimated total non-river time for a given survey date 
    ui_drive_time<-2.5 # units in hours (e.g., daily drive time (hours) to/from duty station to sampling location (i.e., time per shift not spent on the water sampling) 
