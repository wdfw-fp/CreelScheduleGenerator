...
# Specify start and end dates for creel survey schedule (format "yyyy-mm-dd")
  ui_startDate<-as.Date(c("2022-12-01")) 
  ui_endDate<-as.Date(c("2023-02-15")) 
...
# Sample location(s) 
    ...
    ui_water_bodies<-c("Snoqualmie River", "Tokul Creek") # Enter the waterbody(s) that will be surveyed (case sensitive and must match water_body listed in lut$water_body
...
# Index effort count details
  ...
  ui_index_count_time<-1   # [Estimated] time (in hours) it takes to conduct an entire index effort count circuit based on anticipated approach (e.g., number/location of sites, number of creelers conducting index count simultaneously) 
...
# Decide if you want to modify the "survey_start" of shift 1 and specific the latest possible survey_start (time)
  ...
  ui_modify_ss_latest<-c("09:00:00") # Enter the latest "survey_start" time (if "index_time_1" isn't earliest)
